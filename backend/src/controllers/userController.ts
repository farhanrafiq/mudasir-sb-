...
export const changePassword = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Update password
    await UserModel.updateOne({ _id: userId }, { password_hash: passwordHash, temp_pass: false });
    const updatedUser = await UserModel.findById(userId);
    // Get dealer info if dealer
    let dealerId: string | undefined;
    if (req.user!.role === 'dealer') {
      const dealer = await DealerModel.findOne({ user_id: userId });
...
export const updateProfile = async (req: AuthRequest, res: Response): Promise<void> => {
  try {
    ...
    // Update profile
    await UserModel.updateOne({ _id: userId }, { name, username });
    const updatedUser = await UserModel.findById(userId);
    // Get dealer info if dealer
    let dealerId: string | undefined;
    if (req.user!.role === 'dealer') {
